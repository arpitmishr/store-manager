...
function updateTransactionsTable() {
    ...
    sorted.forEach(t => {
        ...
        // This line contains the FIX for undefined. It smartly checks all possible keys
        let itemsStr = 'N/A';
        if (t.items && Array.isArray(t.items)) {
            itemsStr = t.items.map(i => {
                const itemName = i.particulars || i.name || i.itemName || 'Unknown Item';
                const itemQty = i.quantity || i.qty || 1;
                return `${itemName} (x${itemQty})`;
            }).join(', ');
        } else if (t.itemName) {
            // Fallback for very early single-item tests
            itemsStr = `${t.itemName} (x${t.quantity || 1})`;
        }
