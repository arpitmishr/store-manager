...
// Render table based on current search term
function renderInventoryTable() {
    let rowsHtml = [];
    const query = currentInventorySearch.toLowerCase().trim();
    ...
    allInventory.forEach((item) => {
        ...
        rowsHtml.push(`<tr>
            ...
            <td class="px-6 py-4">${itemQty}</td>
            <td class="px-6 py-4">₹${itemPrice.toFixed(2)}</td>
            <td class="px-6 py-4 flex gap-2">
                <button class="btn-edit bg-warning hover:bg-yellow-500 text-white rounded px-3 py-1 transition-colors" data-id="${item.id}" data-name="${itemName}" data-qty="${itemQty}" data-price="${itemPrice}">
                    <i class="fa-solid fa-pen-to-square pointer-events-none"></i>
                </button>
                <button class="btn-delete bg-danger hover:bg-red-600 text-white rounded px-3 py-1 transition-colors" data-id="${item.id}">
    ...
    if (rowsHtml.length === 0) {
        rowsHtml.push(`<tr><td colspan="4" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">No items found matching "${currentInventorySearch}"</td></tr>`);
    }
